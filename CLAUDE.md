...
**الإصدار:** Worker `v3.3.0` · الواجهة `v3.2.0`   ← الاتنين مستقلين، طبيعي يختلفوا
...
✅ **النقل خلص 06-09-2026.** الأداة منشورة **من git بالكامل**:

```
push على main ─┬─→ Cloudflare Workers Builds → الـ Worker لايف
               └─→ GitHub Pages              → الواجهة لايف
```

⛔ **ممنوع لصق كود في داشبورد Cloudflare** — أول push جاي بيمسحه. الريبو ده هو المصدر الوحيد.

ℹ️ **الكود المنشور دلوقتي بيعدّي على esbuild** (bundled · التعليقات كلها متشالة). يعني:
- سطر بصمة المهارات في `index.js` **مش بيوصل** للنسخة المنشورة — ده متوقع، ومصدر الحقيقة للبصمة هو `CLAUDE.md` ده (`ecommoda-skill-versioning` Step 4).
- **مقارنة md5 بين `index.js` والكود المنشور مابقتش شغّالة** — كانت دليل النقل النضيف وقت الرفع اليدوي، وانتهى دورها بالنقل.
...
| `pay` · `refund` | POST | التحصيل والاسترداد الفعلي |
...
⚠️ **`extra_shipping` قيمة تاريخية — مش نوع حيّ.** مسجّلة في `ecommoda-constants` §7 تحت `cod_payment`، لكن كود v3.3.0 مابيكتبهاش خالص. في D1 **157 صف** آخرهم `2026-08-15` من كود أقدم.
✅ **قرار Ahmed 06-09-2026: تتساب زي ما هي** — تفضل في جدول `ecommoda-constants` §7 وتفضل صفوفها في D1. متتوقعش صفوف جديدة بيها، ومتشيلهاش من الجدول.
...
Build watch paths : index.js · wrangler.toml   ← مضيّقة 06-09-2026 (§13-ب)
```
...
## ⚠️ Build watch paths مضيّقة — اقرا ده قبل ما تضيف أي ملف

الـ Worker بينشر من **`index.js` و `wrangler.toml` بس**. أي ملف تاني يتغيّر مابيعملش نشر.
...
🔴 **لو الأداة كبرت وضافت ملف بيعتمد عليه الـ Worker** (`package.json` · فولدر `src/` · ملف config)
**لازم يتضاف للـ Include paths**، وإلا الـ Worker يفضل على نسخة قديمة **من غير أي رسالة خطأ** — نفس شكل الفخ الخامس بالظبط.

⚠️ التضييق بيأثر على **النشر من `main`** بس. بناء الفروع بيشتغل على أي تعديل مهما كان.
...
**✅ التحقق بعد النقل (06-09-2026):** نفس الاستعلام رجّع `payment 3,575` · `refund 75` · `login 74` · `extra_shipping 157` — كلها **أكبر أو تساوي** خط الأساس، وكل الفروق بعد `2026-08-31`. مفيش عمود رجع لورا ومفيش نوع اختفى.

**أقوى دليل على نجاح النقل:** صف `login` بتاريخ `2026-09-06T04:33:23Z` — **بعد ٤٢ ثانية** من أول deploy من git (`04:32:41Z`). يعني موظف حقيقي دخل على الـ Worker المبني من الريبو، والكتابة في D1 نجحت ⇒ `DB` و`WORKER_SECRET` عدّوا التحويل سليمين.
...
آخر مطابقة: 31-08-2026 · `index.js` v3.3.0 · `index.html` v3.2.0
🔴 معلّقة: — لا شيء

> النقل ده **ما لمسش الكود** (`index.js` بايت ببايت من كلاودفلير)، فالبصمة زي ما هي في الكود.
...
- **🟡 الاختبار الإيجابي لـ chip الـ `index.js` لسه ما اتعملش (§13-ب).** التضييق متأكَّد لـ `wrangler.toml` بدليل قاطع: كوميت `5b620dd` (07:32:23) نشر بعد **١٨ ثانية** (07:32:41). لكن `index.js` **لسه مجرَّبش لوحده**. لو الـ chip بتاعه فيه غلطة إملائية أو ناقص، **أي تعديل كود جاي مش هينشر أبدًا من غير أي رسالة**. الاختبار: تعديل سطر تعليق في `index.js` **لوحده** على `main` → لازم يبني وينشر → امسح السطر بعدها.
